        ...
        if(epList)
        {
            ...
            epList.forEach(function(ep)
            {
                ...
                else
                {
                    ...
                    if (ep.LegStartDelay > 0)
                        option = util.format("['leg_delay_start=%d,leg_timeout=%d,origination_caller_id_name=%s,origination_caller_id_number=%s,DVP_OPERATION_CAT=%s,companyid=%s,tenantid=%s,dvp_app_id=%s']", ep.LegStartDelay, ep.LegTimeout, ep.Origination, ep.OriginationCallerIdNumber, 'CONFERENCE', companyId, tenantId, appId);
                    else
